...
function dennis(val) {
    ...
    for (let i = 1; i <= countcategory; i++) {
        let idname = "one" + i;
        if (idname == val.id) {
            document.getElementById("<%=clickedval.ClientID %>").value = document.getElementById(idname).getAttribute("appraisalcode")
            document.getElementById("<%=hiden.ClientID %>").click();
            break;
        }
    }
}
